...
export default function ClientPage() {
  ...
  const [companyConfig, setCompanyConfig] = useState<InsuranceType | null>(null);
  ...
  // Fetch company insurance configuration when both company and type are selected
  useEffect(() => {
    const fetchCompanyConfig = async () => {
      if (selectedCompany && selectedInsuranceType) {
        try {
          // Get the company's insurance configuration
          const response = await fetch(\`/api/insurance?companyId=\${selectedCompany}\`);
          if (response.ok) {
            const data = await response.json();
            if (data.success && data.data) {
              const insuranceConfig = data.data.find((config: InsuranceType) => 
                config.type === selectedInsuranceType
              );
              
              if (insuranceConfig) {
                setCompanyConfig(insuranceConfig);
                setChatEnabled(true);
                setClientData(prev => ({
                  ...prev,
                ...
                // Create field list for the AI prompt
                const fieldDescriptions = insuranceConfig.fields.map(field => {
                  let description = \`\${field.name}: \${field.label}\`;
                  if (field.type === 'select' && field.options) {
                    const optionValues = field.options.map(opt => opt.value).join(', ');
                    description += \` (options: \${optionValues})\`;
                  } else if (field.type === 'range' && field.brackets) {
                    const rangeInfo = field.brackets.map(b => \`\${b.min}-\${b.max}\`).join(', ');
                    description += \` (range: \${rangeInfo})\`;
                  }
                  return description;
                }).join('\\n');

                // Initialize chat with company-specific fields
                setChatHistory([
                  {
                    role: 'system',
                    ...
                    Update the collectedData object as you collect each piece.
                    
                    Start by asking for the first field: \${insuranceConfig.fields[0]?.label || 'your information'}\`
                  }
                ]);

                setMessages(prev => [...prev, {
                  id: Date.now(),
                  text: \`Great! I'll help you get a quote for \${selectedInsuranceType} insurance. Let me ask you a few questions based on \${companies.find(c => c.companyId === selectedCompany)?.companyName || 'this company'}'s requirements.\`,
                  isBot: true,
                }]);
              }
            }
    ...
    fetchCompanyConfig();
  }, [selectedCompany, selectedInsuranceType, companies]);
  ...
  const extractJSON = (text: string): string => {
    ...
    const patterns = [
      ...
      /\\{.*\\}/, // Simple match from first { to last }
    ];
    ...
    // If all else fails, try to extract from last { to end
    const lastBrace = cleaned.lastIndexOf('{');
    if (lastBrace !== -1) {
      return cleaned.substring(lastBrace);
    }
      ...
      if (data.success && data.message) {
        try {
          ...
          if (parsedResponse.answer && parsedResponse.collectedData !== undefined) {
            ...
            // Check if data collection is complete based on company config
            if (companyConfig && parsedResponse.collectedData.completed) {
              // All data collected according to company requirements, get quote
              await getQuote({
                ...clientData,
                ...parsedResponse.collectedData
              });
            } else if (companyConfig) {
              // Check if we have all required fields from the company config
              const requiredFields = companyConfig.fields.map(f => f.name);
              const collectedFields = Object.keys({ ...clientData, ...parsedResponse.collectedData });
              const hasAllFields = requiredFields.every(field => 
                collectedFields.includes(field) && 
                (clientData[field] !== undefined || parsedResponse.collectedData[field] !== undefined)
              );
                  ...
                  <>
                    ...
                    {!selectedCompany || !selectedInsuranceType ? (
                      ...
                      <p className="text-sm text-green-600 dark:text-green-400">
                        ✓ Ready to chat! The AI will ask questions based on this company's requirements.
                      </p>
                    ) : (
                      <p className="text-sm text-yellow-600 dark:text-yellow-400">
                        Loading company requirements...
                      </p>
                    )}
                  </>
                )}
